...
export class SessionWebviewProviderReact {
      ...
      async (message) => {
        switch (message.type) {
          ...
          case 'setSearchCollapsed':
            ...
            break;
          case 'markAgentFinished':
            if (typeof message.agentId === 'string' && message.agentId) {
              this.markAgentFinished(sessionId, message.agentId);
              const updated = await this.loadSessionData(sessionId);
              if (updated) {
                panel.webview.postMessage({ type: 'sessionData', data: updated });
              }
            }
            break;
        }
  ...
  // sessionId → agentId → when the user marked it finished by hand, for
  // agents whose task-notification never reached the transcript. Held in
  // memory only: it acts like a notification at that moment, so an agent
  // resumed afterwards shows up again once it has newer steps.
  private manualFinished = new Map<string, Map<string, string>>();

  private markAgentFinished(sessionId: string, agentId: string): void {
    let m = this.manualFinished.get(sessionId);
    if (!m) {
      m = new Map();
      this.manualFinished.set(sessionId, m);
    }
    m.set(agentId, new Date().toISOString());
  }
  ...
  private async loadSessionData(sessionId: string): Promise<SessionDetail | null> {
    try {
      ...
      this.parserService.linkSubagentsToParents(session.steps, subagents, session.agentFinishedAt);
      const manual = this.manualFinished.get(sessionId);
      if (manual) {
        for (const sub of subagents) {
          if (sub.finished !== false) continue;
          const at = manual.get(sub.agentId);
          if (at && ParserService.finishedBy(sub, at)) sub.finished = true;
        }
      }
