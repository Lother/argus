...
// sessionId → agent ids the user marked finished by hand, for agents whose
// task-notification never reached the transcript. Survives reloads.
const MANUAL_FINISHED_KEY = 'argus.session.manualFinishedAgents';
...
export class SessionWebviewProviderReact {
        ...
        switch (message.type) {
          ...
          case 'markAgentFinished':
            if (typeof message.agentId === 'string' && message.agentId) {
              await this.markAgentFinished(sessionId, message.agentId);
  ...
  private getManualFinished(sessionId: string): string[] {
    const all = this.context.globalState.get<Record<string, string[]>>(MANUAL_FINISHED_KEY, {});
    const ids = all[sessionId];
    return Array.isArray(ids) ? ids : [];
  }

  private async markAgentFinished(sessionId: string, agentId: string): Promise<void> {
    const all = { ...this.context.globalState.get<Record<string, string[]>>(MANUAL_FINISHED_KEY, {}) };
    const ids = new Set(this.getManualFinished(sessionId));
    ids.add(agentId);
    all[sessionId] = Array.from(ids);
    await this.context.globalState.update(MANUAL_FINISHED_KEY, all);
  }
  ...
  private async loadSessionData(sessionId: string): Promise<SessionDetail | null> {
    try {
      ...
      const manualFinished = new Set(this.getManualFinished(sessionId));
      for (const sub of subagents) {
        if (manualFinished.has(sub.agentId)) sub.finished = true;
      }
